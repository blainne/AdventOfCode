...
let findMaxMinDiff nums =        
    let updateMaxMin (max, min) = 
        function
        | both when (both > max && both < min) -> (both, both)
        | less when less < min -> (max, less)
        | more when more > max -> (more, min)            
        | _ -> (max, min) 

    nums
    |> Array.fold updateMaxMin (Int32.MinValue, Int32.MaxValue)
    |> fun (max, min) -> max - min

let findEvenDivision nums =
    let rec checkOneNumber num rest =
        match rest with
        | [] -> None
        | h::t -> 
            if num % h = 0 
            then Some(num, h)
            else checkOneNumber num t

    let rec findDivision soFar num toCheck =
        match checkOneNumber num (soFar@toCheck) with
        | Some(num, h) -> num/h
        | None -> 
            let newNum::newToCheck = toCheck
            findDivision (num::soFar) newNum newToCheck

    let (num::rest) = List.ofArray nums
    findDivision [] num rest     

let calcResult inp =
    inp
    |> preprocessInput
    |> Array.sumBy findMaxMinDiff

let calcResult2 inp =
    inp
    |> preprocessInput
    |> Array.sumBy findEvenDivision

let input = """4347 3350 196 162 233 4932 4419 3485 4509 4287 4433 4033 207 3682 2193 4223
...
let result1 = calcResult input      
let result2 = calcResult2 input